...
# RANSAC 파라미터 설정
ransac_threshold = 4
H, mask = cv2.findHomography(img2, dst2, cv2.RANSAC, ransac_threshold)
...
if num >= num_threshold:
    inlier_points = np.array([dst2[i][0] for i in range(len(mask)) if mask[i] == 1])
    x, y, w, h = cv2.boundingRect(inlier_points)
    img = cv2.rectangle(img, (x  , y), (x + w +3, y + h+3), (0, 0, 255), 2)
